...
const char * const BITCOIN_CONF_FILENAME = "gull.conf";
const char * const BITCOIN_PID_FILENAME = "gulld.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "gull" is a composite category enabling all gull-related debug output
            if(ptrCategory->count(string("gull"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\gullCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\gullCore
    // Mac: ~/Library/Application Support/gullCore
    // Unix: ~/.gullcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "gullCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/gullCore";
#else
    // Unix
    return pathRet / ".gullcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty gull.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override gull.conf
